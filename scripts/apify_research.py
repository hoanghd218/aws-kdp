...
def _get_tokens() -> list[str]:
    """Return the APIFY token pool. APIFY_API_TOKEN may be a comma-separated list;
    tokens are tried in order so an exhausted key (HTTP 403 monthly limit) rotates
    to the next one automatically."""
    raw = env("APIFY_API_TOKEN") or ""
...
def _actor_run_sync(actor_id: str, payload: dict, timeout: int = 180) -> list[dict]:
    ...
    Rotates through the token pool on auth/quota errors (401/402/403)."""
    tokens = _get_tokens()
    ...
    for i, token in enumerate(tokens):
        ...
        try:
            ...
            if exc.code in (401, 402, 403) and i < len(tokens) - 1:
                print(f"⚠️  Apify token #{i+1} failed ({exc.code}); rotating to #{i+2}…", file=sys.stderr)
                continue
            print(f"❌ Apify {last_err}", file=sys.stderr)
            sys.exit(2)
        except error.URLError as exc:
            print(f"❌ Apify network error: {exc}", file=sys.stderr)
            sys.exit(2)
    print(f"❌ All Apify tokens exhausted. Last error: {last_err}", file=sys.stderr)
    sys.exit(2)
