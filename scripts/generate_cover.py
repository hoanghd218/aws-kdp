...
def _backup_existing_cover(paths) -> None:
    """Copy any existing cover file into output/{theme}/backups/ before overwriting.

    Keeps the previous cover.png/cover.pdf with a timestamp so a rebuild never loses
    the version that may already be uploaded to KDP. Mirrors the chatgpt cover builder.
    """
    import shutil
    for p in paths:
        src = Path(p)
        ...
        stamp = int(time.time())
        dest = backup_dir / f"{src.stem}_backup_{stamp}{src.suffix}"
        shutil.copy2(src, dest)
        print(f"  Backed up old cover: {dest}")
...
def build_cover(
    ...
    # Back up any existing cover before overwriting it.
    _backup_existing_cover([png_path, pdf_path])
    ...
    # Auto safe-zone check — text too close to the edge is the #1 cover reject.
    try:
        from check_cover_safezone import check_safezone
        res = check_safezone(png_path)
        print(f"  Safe-zone overlay: {res['overlay']}")
        if res["flagged"]:
            print(f"  ⚠️  Outlier ink on edge(s): {', '.join(res['flagged'])} — likely text in the margin.")
        print("  👉 Open cover_safe_check.png and confirm NO text/badge crosses the RED line "
              "(decoration crossing it is fine).")
