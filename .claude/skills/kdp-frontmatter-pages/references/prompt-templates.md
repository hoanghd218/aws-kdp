...
get a misspelled word — that's expected. After the user generates the images,
inspect them; if small copy is wrong, either ask them to regenerate, or the
...
## 2.txt — THIS BOOK BELONGS TO  (always required — adults AND kids)
```
"THIS BOOK BELONGS TO" page for a <KIDS|ADULT> coloring book — <SQUARE|PORTRAIT 3:4>,
...
3) [KIDS ONLY] Shorter centered line with a blank rule: "AGE: ____".
   [ADULTS] Omit the age line — leave clean white space instead.
4) CENTER-BOTTOM one big adorable <THEME mascot or motif> illustration with thick black outlines
   + soft gray shading (e.g. <mascot> with <theme accessory/item>),
...
> **Adults vs kids:** For adult books omit "AGE: ____" and use theme motifs
> (e.g. a kawaii cat with a teacup) instead of a child-facing mascot if the
> book style calls for it. Everything else is identical.
