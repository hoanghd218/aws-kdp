...
illustrations. The "This Book Belongs To" page is ALWAYS included for both
adults and kids books.

Expected images in  output/<theme>/frontmatter/ :
    1.png  -> Title page            (REQUIRED)
    2.png  -> This Book Belongs To   (REQUIRED — always included)
    3.png  -> Thank You page         (REQUIRED)

The illustrations should already contain their own text (baked in by the image
model). This script only places them, adds a styled copyright page, keeps
...
def main():
    ...
    for required in (title_png, belongs_png, thanks_png):
    ...
    print(f"Coloring pages: {len(images)} | size: {size} | author: {author} | audience: {audience}")

    # --- front-matter page list (each entry rendered on its own page) ---
    # Always: title → copyright → belongs-to (for ALL books, adults and kids)
    front = ["title"]
    ...
    def copyright_page():
        ...
        author_line = f"Copyright © {year} {author}. All rights reserved."

        # Decorative top rule
        c.setLineWidth(0.6)
        c.line(x + w * 0.25, y + h * 0.62, x + w * 0.75, y + h * 0.62)

        # Copyright heading bold
        c.setFont("Arial-Bold", 11)
        c.drawCentredString(cx, y + h * 0.56, author_line)

        # Thin separator
        c.setLineWidth(0.3)
        c.line(x + w * 0.25, y + h * 0.53, x + w * 0.75, y + h * 0.53)
        ...
        c.setLineWidth(0.6)
        c.line(x + w * 0.25, y + h * 0.12, x + w * 0.75, y + h * 0.12)
        c.showPage()
    ...
    print(f"Next: python scripts/pdf_qc.py --pdf {out_path} --trim {size} --require-even-pages")
